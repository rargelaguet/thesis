...
% MANUSCRIPT
% MOFA identifies both technical and biological sources of variation, including factors that are linked to clinical outcome, e.g. time to next treatment. 
% When considering individual factors as single predictors for time to treatment several of them show highly significant associations (Figure 4b), among those factor 1 (IGHV status) as well as factors 7 and 8. When splitting patients into two groups based on each of those factor we observe a clear stratification of patients (Figure 4c). When annotating the latter two factors, associations to pre-treatment and TP53/del17p mutational status of the patients are observed. Methylation and drug response data link factor 7 to cancer-related genes and the response to the drug rotenone (Figure S8). 

% We conjectured that proper integration of the various omic layers would be beneficial for predicting clinical outcome compared to using only single omics or naïve integration. This was confirmed when predicting the time until a patient needs to be treated using a multivariate Cox model. We compared the prediction performance measured by Harrell’s C-Index in a stratified cross-validation procedure with five folds. The prediction made by MOFA show a high C-Index of C=0.78. In particular, MOFA factors outperform multivariate models based on the same number of principal components both from single-omic data (C=0.68-0.72), individual genetic markers (C=0.66) as well the concatenated data matrix (C=0.74) (Figure 4a). Similar results were obtained when including all features instead of its principal components (Figure S13). 
...
% We also assessed the prediction performance when combining the 10 MOFA factors in a multivariate Cox regression model.
% Notably, this model yielded higher prediction accuracy than models using components derived from conventional PCA (Fig 4C), individ- ual molecular features (Appendix Fig S20) or MOFA factors derived from only a subset of the available data modalities (Appendix Fig S8B and D; assessed using cross-validation, Materials and Methods). The predictive value of MOFA factors was similar to clinical covari- ates (such as lymphocyte doubling time) that are used to guide treatment decisions (Appendix Fig S21).
...
Jointly, the combination of Factors 1 and 2 reconstruct the full differentiation trajectory from naive pluripotent cells to differentiated cells \Cref{fig:XX}. When applying popular integrative clustering algorithms \cite{Wang2014,Shen2009,Moi2013}, the trajectory is not recovered \Cref{fig:XX}, illustrating the importance of learning continuous latent spaces before applying clustering methods.
