...
\end{align*}
As in \cite{Tipping1999}, the loadings and the variance of the noise are assumed to be non-probabilistic parameters, whereas the factors are probabilistic unobserved variables (\Cref{fig:graphical_CCA}). This yields the following likelihood functions:
...
Finally, notice that if $M=1$ the model reduces to Bayesian PCA (\Cref{section:bayesian_pca}). However, if $M=2$ the model does not reduce to Bayesian CCA, because in the GFA setting some factors are also allowed to capture the intra-specific variability.
