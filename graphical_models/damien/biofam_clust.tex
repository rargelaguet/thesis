...
\begin{tikzpicture}
  ...
  \node[obs]   (Y) {$y_{n,d}^m$};
  \node[latent, above=of Y, xshift=-2.5cm] (Z) {$z_{n,k}$};
  \node[latent, above=of Y, xshift=1.5cm] (W) {$w_{k,d}^m$};
  \node[latent, xshift=1.5cm] (Tau) {$\tau_{d}^m$};

  \node[opacity=0.4,latent, xshift=-2.5cm] (Tau2) {$\tau_{n}$};
  ...
  % cluster plate
  \node[latent, above=of What, xshift=-1.3cm] (muW) {$\mu^m_{k, c}$};
  \node[latent, above=of Zhat, xshift=1.3cm] (muZ) {$\mu_{k, c}$};
  \edge {muZ} {Zhat};
  \edge {muW} {What};
  ...
  % Plates
  % cluster plate
  \plate[] {plateK} {(Z)(W)(SZ)(Zhat)(SW)(What)(ThetaZ)(ThetaW) (muW)(plateC)} {$K$};
